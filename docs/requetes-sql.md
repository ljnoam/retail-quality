...
**Reproduction.** Avec Application Default Credentials et un `GOOGLE_CLOUD_PROJECT` autorisé à lancer des jobs BigQuery, exécuter successivement `python src/extract_bigquery.py discover`, `compare`, puis `extract`. Les commandes complètes sont dans le README. Le jeu public peut évoluer ; pour rejouer exactement les mêmes lignes hors connexion, utiliser le CSV figé et son SHA-256 dans `evidence/mesures_des_requetes/extractions_integrity.json`.
